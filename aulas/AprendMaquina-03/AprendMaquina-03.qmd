...
-   Quanto maior a dimensionalidade do conjunto de dados, mais detalhes temso sobre os dados, mas também aumenta a complexidade computacional e o consumo de recursos.
...
-   Frequência e modea são medidas úteis para descrever atributos categóricos.
...
-   No R, um resumo das estatísticas pode ser obtido com a função [**summary()**]{.dstq}.
    ...
    -   Se quisermos analisar atributos específicos, podemos usar as funções do pacote [**dplyr**]{.dstq}.
        ...
        -   [**mutate:**]{.dstq} para moficiar variáveis.
...
-   Uma das visualizações mais usadas é o histrograma, que permite mostrar a dispersão e a assimetria de um atributo.
...
-   Com um gráfico de barras empilhadas, é possível mostrar como um valor total pode ser dividido em partes ou destacar a importância de cada parte em relação ao valor total. Ele ajuda a responder pergutnas como:
...
-   Se o ruído for um problema para o método, podemos usar algum método de suavização:
...
-   Como exemplo, em um conjunto de dados ordenados, ao dividir em três intervalos, substituímos os valores em cada intervalo pela média do intervalo, resultando em uma versão suavizada dos dados.
-   Exemplo: $$ 4, 8, 9, 15, 21, 21, 24, 25, 26, 28, 29, 34 $$ $$ \color{blue}{4,8, 9,15}, {21, 21, 24, 25}, \color{red}{26, 28, 29,34} $$ $$ \color{blue}{9, 9, 9, 9}, 23, 23, 23, 23, \color{red}{29, 29, 29, 29} $$
...
-   Outro problema é que o aprendizado com dadso desbalanceados pode resultar em modelos com precisão preditiva enganosamente otimista (mais fáci clssificar todo mundo em uma classe, já que poucas observações pertencem à outra).

## Limpeza dos dados
### Desbalanceamanto de classe

Métodos para resulver o problema:
...
-   [Reamostragem dos dados:]{.dstq} subamostrar a classe majoritária (*under-sampling*) ou a superamostrar da classe minoritária (*over-sampling*), inclusive com a geração de exemplos sintéticos, como feito pelo algoritmo *Synthetic Minority Over-sampling Technique* (SMOTE).
...
- Como o maior valo é 1269.6, vamos dividir por $10^4$
...
- Os dados originais são trasnformados para um novo intervalo definido pelos limites inferior e superior especificados pelo usuário, frequentemente 0 e 1.
...
- As transformações anteriores são boas quandoos dados são simétricos.
...
-  Algumas vezes precisamos reduzir os número de observações, o número de variáveis, ou ambos para utlização de um modelo de aprendizado de máquina.
